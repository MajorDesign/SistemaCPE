...
def login(request: Request, data: dict):
    """
    Login do usuário
    Body: { "credential": "email_ou_username", "password": "senha" }
    """
    ...
    try:
        credential = (data.get("credential") or data.get("email", "")).strip().lower()
        password = data.get("password", "")

        if not credential:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Email ou usuário obrigatório")
        if not password:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Senha obrigatória")
        ...
        if not user:
            print(f"[AUTH/LOGIN] ✗ Usuário não encontrado: {credential}")
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Email ou senha incorretos")

        if not verify_password(password, user["password_hash"]):
            print(f"[AUTH/LOGIN] ✗ Senha incorreta para: {credential}")
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Email ou senha incorretos")

        if user["is_active"] != 1:
            print(f"[AUTH/LOGIN] ✗ Usuário inativo: {credential}")
